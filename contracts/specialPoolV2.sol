...
contract RGPSpecialPool is Ownable {
    ...
    function stake(uint256 _quantity, address _referral) public {
        ...
        if (_userData.tokenQuantity == 0) {
            ...
            IBEP20(TOKEN).transferFrom(_msgSender(), devAddress, Fee);
            totalFeeEarn += Fee;
        }
    ...
    function resetRates(uint256 _entryRate, uint256 _thirtyDaysRate, uint256 _yearlyRate) external onlyOwner {
        ENTRY_RATE = _entryRate;
        DAYS30_RATE = _thirtyDaysRate;
        YEAR_RATE = _yearlyRate;
        emit rateUpdated(_entryRate, _thirtyDaysRate, _yearlyRate);
    }
